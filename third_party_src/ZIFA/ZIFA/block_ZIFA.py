...
def fitModel(Y, K, singleSigma=False, n_blocks=None, p0_thresh=.95):
	...
	keep_Y_indices = (np.abs(Y) < 1e-6).mean(axis=0) <= p0_thresh
	Y = Y[:, keep_Y_indices]
	...
	while results is None:
		...
		keep_Y_indices_tmp = Y_is_zero.mean(axis=0) < new_max_zero_frac
		keep_Y_indices[keep_Y_indices] = keep_Y_indices_tmp
		Y = Y[:, keep_Y_indices_tmp]
		print('After filtering out genes with too many zeros, %i samples and %i genes' % Y.shape)

		results = runEMAlgorithm(Y, K, singleSigma=singleSigma, n_blocks=n_blocks)

	return results, keep_Y_indices
